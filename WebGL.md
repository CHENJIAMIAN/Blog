...
- **片元着色器中的 vPosition 不是直接来自任何一个特定的顶点 它是通过GPU自动进行的插值计算得到的 这种插值确保了三角形内部的平滑过渡**
```javascript
...
### 经验
1. 处理透明度时**组合模型的优越性**：
	1. 由于透明模型相互重叠，无法确保正确的从后到前的渲染顺序，导致视觉混乱和不可预测的渲染效果
	2. 将多个透明物体组合为一个整体进行渲染，可以显著提高渲染的准确性和一致性。