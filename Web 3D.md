...
1. 无损与有损压缩的结合
	1. 无损压缩
		1. 在DRACO的无损压缩中，原始数据的每一个点和三角形都被精确地保存下来，没有数据损失。其原理是基于点云压缩和几何压缩技术实现的。DRACO将同一区域内的点打包成局部网格，并使用Delta编码和算术编码技术对每个点的空间位置进行压缩。在解码时，DRACO重新拼接出原始数据。
	2. 有损压缩
		1. DRACO的有损压缩原理是基于点云压缩和模型降采样技术实现的。在压缩过程中，DRACO会移除冗余和细节信息，从而减少数据量。这种降低数据冗余和细节的方式会导致信息的损失，在一定程度上影响原始的数据完整性。
3. 熵编码
	1. 该技术可以将模型中不同类型的数据进行分组，然后把每一组数据挑选出一些参考数据，用较小的数据表示所有的数据，从而减小整个模型的数据量。
	2. 数据熵编码技术的一个常用形式是霍夫曼编码，它可以将数据中出现频率较高的符号用较短的编码表示，而把出现频率较低的符号用较长的编码表示，从而在不增加数据量的情况下，实现了数据压缩的目的。
