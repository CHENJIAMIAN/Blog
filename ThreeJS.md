...
4. 算法
	...
	3. ACES Filmic 算法：ACES Filmic 是一种基于电影工业标准的色调映射算法，旨在提供更加真实和自然的色彩和光照表现。它通过非线性映射和曝光度调整来处理高动态范围图像，以实现更好的视觉效果。**伽马校正**可以作为其中的一部分，用于调整图像的颜色和亮度
...
6. SSAO：屏幕空间环境光遮蔽，用于模拟物体之间的阴影和光线反射效果。
7. SSR：屏幕空间反射，用于模拟物体表面的反射效果。
8. DOF：景深效果，用于模拟镜头聚焦和模糊效果。
9. ChromaticAberration：色差，用于模拟镜头成像时不同颜色光线的折射效果。
10. Vignetting：暗角效果，用于在图像边缘添加暗影效果。
11. BlurEdge：边缘模糊，用于模拟镜头成像时边缘模糊效果。
12. Film：胶片效果，用于模拟老式胶片的颜色和质感。
13. Glow：发光效果，用于模拟物体周围的光晕效果。
14. Lensflare：镜头光晕，用于模拟镜头成像时光晕效果。
