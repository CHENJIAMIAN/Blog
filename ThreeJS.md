...
7. SSR：屏幕空间反射，用于模拟物体表面的反射效果。
	...
	- **SSR后处理的使用**：
	    ...
	    - 缺点：仅限于相机看到的内容。
1. DOF：景深效果，用于模拟镜头聚焦和模糊效果。
2. ChromaticAberration：色差，用于模拟镜头成像时不同颜色光线的折射效果。
3. Vignetting：暗角效果，用于在图像边缘添加暗影效果。
4. BlurEdge：边缘模糊，用于模拟镜头成像时边缘模糊效果。
5. Film：胶片效果，用于模拟老式胶片的颜色和质感。
6. Glow：发光效果，用于模拟物体周围的光晕效果。
7. Lensflare：镜头光晕，用于模拟镜头成像时光晕效果。
