...
帮我写一个页面, 页面结构和内容如下:
1.第一行内容为原型的头像+灰色文本"保证金"+黑色文本"0U"+Spacer+蓝色文字蓝色边框白色背景迷你大小的"划转"按钮+Spacer+灰色文本"利息"+橘色加粗带下划线的文本"0"+灰色文字灰色边框白色背景迷你大小的"领取"按钮。(为相邻的部件加上合理舒适的间隔)
2.第二行内容为列1+Spacer+列2, 列1有两行, 列1的第一行内容为灰色文本"搬砖余额(RMB)"+灰色线框的表示"睁开的眼睛"的图标, 列1的第二行内容为大字体加粗的文本"0", 列2是一个包含有两列的Row, 其中第一列有两个按钮,一个是橘色文字橘色边框白色背景迷你大小的"转入"按钮,个是蓝色文字蓝色边框白色背景迷你大小的"转出"按钮, 第二列是正常大小的蓝底白字按钮"开始搬砖"
3.第三行是分隔符
4.第四行是灰色文本"我的下级"+黑色带下划线文本"0人"+灰色文本"(返利"+橘色文本"0.0%"+灰色文本")"+Spacer+灰色文本"已返利"+橘色带下划线文本"0"+灰色文字灰色边框白色背景迷你大小的"领取"按钮
5.第五行是灰色文本"分享链接"+蓝色文本"https://xxx.xxx"+Spacer+蓝色线框的代表复制的图标
6.第六行是一个占满以下空余位置的Tab标签组件, 有4个Tab, 分别是进行中, 成功, 失败/取消, 全部,  而且在全部之后有个筛选按钮, 点击它会自动把Tab标签栏固定到顶部与appbar相邻的地方并向下弹出drawer让用户选择筛选项, 筛选项分两类, 一类是订单状态,可单选, 分别有"全部","进行中","交易成功","交易失败","投诉中","维权中" 6个单选项, 另一类是交易金额,可输入最小和输入最大以限制范围, drawer的最下方是蓝色边框白色底的"重置"按钮和蓝色的确定按钮