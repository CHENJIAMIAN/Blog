...
- 消息的筛选有问题
- 按返回键不能返回
- 1. 消息-进入维权- 返回页面不对（返回了搬砖页面）， 应该返回消息页面,   同样从订单 进入投诉也是一样不对， 返回了搬砖页面
2. 区块流水、游戏钱包明细  时间格式修改（yyyy-MM-dd HH:mm:ss）, 游戏钱包明细金额保留2位小数
3. 转入游戏钱包  去掉（公链协议）和 下面的重要提示， 转入前搬转余额改为  转入前余额， 并且余额要跟选择钱包一致（目前余额总是0）
4. 我的挂单， 取消 改成  按钮形式， 类似订单的 取消按钮
5. 消息模块的界面切换改成渐变效果（跟其他页面一致）
6. 游戏钱包余额保留2位小数
7.  进入系统， 默认显示  游戏钱包， 游戏钱包-订单， 默认显示全部
8.  增加游戏钱包余额 转入USDT  - 游戏转入币种(USDT){{HOST}}/balance/gameTransferCoin
