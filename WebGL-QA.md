...
光照模型 (Lighting Models)  
    ├── 基于经验的光照模型 (Empirical Lighting Models)  
    │   ├── Lambertian (漫反射)  
    │   ├── Phong (高光反射)  
    │   └── Blinn-Phong (改进的高光反射)  
    └── 基于物理的光照模型 (Physically Based Lighting Models)  
         │   ├──  PBR (基于物理的渲染)  
         │   │   ├── 金属/粗糙度工作流  
         │   │   ├── 反射率/粗糙度工作流  
         │   │   └──  基于微表面的BRDF模型  
         ...
         │      │  └── Cook-Torrance (微表面模型)  
         │      ├── BTDF (双向透射分布函数)  
         │      └── Principled BSDF (多功能/迪士尼 BSDF)  

Principled BSDF  
    ├── 微表面理论(Microfacet Theory):  
    │   ├── 表面被建模成由微小的、镜面反射的小平面组成的集合  
    │   │   ├── 每个小平面都服从菲涅尔方程和 Snell 定律  
    │   │   └── 微表面的分布函数(例如，GGX 或 Beckmann)决定了表面的粗糙度  
    │   ├── 菲涅尔方程(Fresnel Equation):  
    │   │   ├──  描述了光线在不同介质之间传播时反射和折射的比例  
    │   │   └── 反射率随入射角度变化  
    │   └──  能量守恒:  
    │       ├──  确保反射和折射的总能量不超过入射能量  
    │       └──  避免出现不真实的过亮效果  
    └──  基于BRDF(双向反射分布函数)和BTDF(双向透射分布函数)的数学模型:  
        ├── BRDF:  
        │    ├── 漫反射项  
        │    │    └── 通常使用 Lambertian 模型或改进版本  
        │    ├── 镜面反射项  
        │    │    └── 通常使用 GGX, Beckmann等微表面模型  
        │    └── 基于物理的反射模型 (PBR的核心，包含了菲涅尔效应和微表面模型)  
        └── BTDF:  
             ├── 透射项  
             │  └── 使用折射率控制透明度  
             └── 基于物理的折射模型 (PBR中的透明材质处理)
```
...
解决方法是使用法线矩阵来变换法线向量。法线矩阵是模型视图矩阵的**逆转置矩阵**，可以保证在进行任何线性变换时，法线的方向不会发生改变。实现方法如下：
...
- **逆转置矩阵**和**伴随矩阵**有密切关系: `(M⁻¹)ᵀ = adj(M)/det(M) `,  即一个矩阵的逆转置矩阵可以通过其伴随矩阵和行列式来计算
- **伴随矩阵的几何意义：** 伴随矩阵可以被视为是对原矩阵的一种“伴随”变换，这种变换会保留原矩阵变换中的一些性质，比如方向。

1. 如果使用变换矩阵 `M` 变换一个切向量 `t`，得到 `t' = M*t`
2. 如果使用另外一个矩阵 `N` 变换法向量 `n`，得到 `n' = N*n`
3. 因为法向量和切向量垂直，所以点积 `n.t=0`
4. 变换后，仍然要保证法向量和切向量垂直，所以点积 `n'.t'=0`
5. 即 `(N*n) . (M*t) = 0`
6. 所以 `(N*n)ᵀ*(M*t) = 0`
7. 所以 `nᵀNᵀ M t = 0`
8. 要使得等式成立，那么 `NᵀM = I` (I是单位矩阵)，即 `Nᵀ = M⁻¹` ， 所以 `N = (M⁻¹)ᵀ`
9. 这个推导看起来更像是数学推导，但实际上，它也是在从“维持垂直”这个几何关系进行推导。
### 摄像头矩阵转换的WebGL的实现
