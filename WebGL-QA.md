...
切线空间描述3D物体表面的几何性质（例如：法线方向），将其转化为相对屏幕表现更加友好的2D形式，使其变得可渲染出来。在切线空间，我们使用三组轴：法向量（Normal）、切线向量（Tangent）和副切向量（Bitangent）。在着色器中，我们通常需要将所有缓冲区数据从世界坐标系下的参考帧中变换到切线空间。

## 除了切线空间还有什么空间

除了切线空间，还有世界空间、视图空间以及投影空间。

世界空间描述的是物体在三维空间中的位置和大小，是以场景中心为原点的坐标系。
视图空间描述相机观察场景时的状态，是以眼睛所在的位置为原点的坐标系，可以理解为相对于相机的坐标系。
投影空间描述的是几何图形如何投射到屏幕上，也称裁剪空间，是一种将三维空间内坐标转化为画面二维坐标的虚拟空间

## 切线空间与屏幕坐标有什么关系

切线空间（Tangent Space）是一个由模型的法向量（Normal）、切线向量（Tangent）和副切向量（Bitangent）共同构成的三维空间，是为了在简化渲染方程的同时仍得到尽可能真实的表现而引入的。在这个空间中，光照、纹理等都可以进行计算和展示。
