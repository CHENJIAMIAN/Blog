...
1. 模板管理 里面 各种量 就是 测点
2. 模板管理 实例化到 采集配置里
3. 在实时监控-实时数据 看测点数据的实时变化

1. 区域/设备 变量名称潜规则, 区域命名为p_area, 设备命名为dev
2. 连库信息 中的占位符, 是用来替换的, 被上一级的 连库信息(即区域/设备唯一编码)  替换掉
3. 最终拼接完成的连库信息(即测点的唯一编码) ,用来向后台请求测点实时0数据
4. 列绑定 点绑定 弹框-变量名称 就是占位符的名称