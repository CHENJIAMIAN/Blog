...
this._xSquared = x * x;
```
> 这个建议的原理是优化 JavaScript 引擎的内存管理和执行。当你在一个构造函数中写入属性时，JavaScript 引擎会动态地创建一个新的对象并为其分配内存，这个对象最终会成为构造函数创建的实例对象。当一个属性被写入值时，JavaScript 引擎不仅需要分配内存，还需要为每个实例对象保存这个属性值。如果在构造函数中重复读取该属性的值，会导致引擎解析和访问对象的过程非常缓慢。
> 
> 这个建议的底层原理是“隐藏类”和“预编译器”，在生成执行代码之前，JavaScript 引擎会根据代码的结构生成隐藏类，这样可以避免使用字典模式，即每个对象都有自己的属性集，而是将同样类型的属性打包在隐藏类中。这样一来，每个属性都可以直接映射到内存地址，访问起来的速度就变得飞快了。
> 
> 更底层的原理是，JavaScript 引擎的内存分配和访问方式是基于预测的。它会在运行时推断代码的数据类型并将其优化为快速指针操作。但是，如果在构造函数中混用属性访问和赋值，就会导致该对象进入字典模式，实际上会使引擎退化为更老的内存管理和执行模式，导致内存和执行问题。因此，为了让 JavaScript 引擎尽可能地利用更先进，更快速的内存和执行方式，我们应该遵循上述建议。
